  ...
  callWithJQuery(function ($) {
    ...
    pivotTableRenderer = function (pivotData, opts) {
      ...
      totalRow = function (table) {
        ...
        renderedRow.push(renderedCell);
        tr.appendChild(th);
        if (opts.totalColumnPlacement === "left") {
          // Generate column total last row
          totalColumnTotal(tr, renderedRow);
        }
        for (j in colKeys) {
        ...
        if (opts.totalColumnPlacement === "right") {
          // Generate column total last row
          totalColumnTotal(tr, renderedRow);
        }
        table.appendChild(tr);
        renderedValues.push(renderedRow);
      };
      for (j in colAttrs) {
        ...
        if (parseInt(j) === 0 && rowAttrs.length !== 0) {
          ...
          renderedCell = {class: "header", colspan: rowAttrs.length, rowspan: colAttrs.length};
        ...
        th = document.createElement("th");
        renderedCell = {value: c, class: "header"};
        th.className = "pvtAxisLabel";
        th.innerHTML = c;
        renderedRow.push(renderedCell);
        tr.appendChild(th);
        if (opts.totalColumnPlacement === "left") {
          // Generate column total header
          totalColumnHeader(j, tr, renderedRow);
        }
        for (i in colKeys) {
          ...
          if (x !== -1) {
            th = document.createElement("th");
            renderedCell = {value: colKey[j], class: "header", colspan: x};
            th.className = "pvtColLabel";
            th.innerHTML = colKey[j];
            th.setAttribute("colspan", x);
            if (parseInt(j) === colAttrs.length - 1 && rowAttrs.length !== 0) {
              th.setAttribute("rowspan", 2);
              renderedCell.rowspan = 2;
            }
            renderedRow.push(renderedCell);
            tr.appendChild(th);
          }
      ...
      if (rowAttrs.length !== 0) {
        ...
        for (i in rowAttrs) {
          ...
          r = rowAttrs[i];
          th = document.createElement("th");
          renderedCell = {value: r, class: "header"};
          th.className = "pvtAxisLabel";
          th.innerHTML = r;
          renderedRow.push(renderedCell);
          tr.appendChild(th);
        ...
        th = document.createElement("th");
        renderedCell = {class: "header total"};
        if (colAttrs.length === 0) {
          th.className = "pvtTotalLabel";
          th.innerHTML = opts.localeStrings.totals;
          renderedCell.value = opts.localeStrings.totals;
        }
        renderedRow.push(renderedCell);
        tr.appendChild(th);
        renderedValues.push(renderedRow);
      ...
      for (i in rowKeys) {
        ...
        for (j in rowKey) {
          ...
          if (x !== -1) {
            th = document.createElement("th");
            renderedCell = {value: txt, class: "header", rowspan: x};
            th.className = "pvtRowLabel";
            th.innerHTML = txt;
            th.setAttribute("rowspan", x);
            if (parseInt(j) === rowAttrs.length - 1 && colAttrs.length !== 0) {
              th.setAttribute("colspan", 2);
              renderedCell.colspan = 2;
            }
            renderedRow.push(renderedCell);
            tr.appendChild(th);
          }
        ...
        if (opts.totalColumnPlacement === "left") {
          // Generate column total values
          totalColumnValues(rowKey, tr, renderedRow);
        }
        for (j in colKeys) {
          if (!hasProp.call(colKeys, j))
            continue;
          ...
          renderedCell = {value: aggregator.format(val), class: "value" + tdClassValue(val)};
          td.className = "pvtVal row" + i + " col" + j + tdClassValue(val);
      ...
      if (opts.totalRowPlacement === "bottom") {
        totalRow(result);
      }
      result.setAttribute("data-numrows", rowKeys.length);
      result.setAttribute("data-numcols", colKeys.length);
      renderedData = renderedValues;
      return result;
    };
    /*
    ...
    $.fn.pivotUI = function (input, inputOpts, overwrite, locale) {
      ...
      defaults = {
        ...
        totalColumnPlacement: "right",
        totalRowPlacement: "bottom",
      ...
      try {
        ...
        rendererControl = $("<td class='pvtRendererControl'>");
        ...
        for (x in ref1) {
          if (!hasProp.call(ref1, x))
            continue;
          $("<option>").val(x).html(ref1[x].label).appendTo(renderer);
        }
        ...
        fn = function (c) {
          ...
          showFilterList = function (e) {
            valueList.css({
              ...
              top: $(this).height() + $(this).position().top
            }).toggle();
        ...
        for (i in shownAttributes) {
          if (!hasProp.call(shownAttributes, i))
            continue;
          c = shownAttributes[i];
          fn(c);
        }
        ...
        for (x in ref2) {
          if (!hasProp.call(ref2, x))
            continue;
          aggregator.append($("<option>").val(x).html(ref2[x].label));
        }
        ...
        refreshDelayed = (function (_this) {
          return function () {
            ...
            numInputsToProcess = (ref5 = opts.aggregators[aggregator.val()].fn([])().numInputs) != null ? ref5 : 0;
            vals = [];
            ...
            subopts.vals = vals;
            subopts.aggregator = opts.aggregators[aggregator.val()].fn(vals);
